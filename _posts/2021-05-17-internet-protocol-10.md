  ...
  - Windows에서 APIPA(Automatic Private IP Addressing)로 부름
    - 169.254.0.0/16에서 무작위 선정하여 인터페이스 할당
      - 커널이 마음대로 난수를 돌려서 주소를 정한다.
        - 하필이면 같은 링크에 있는 여러개의 호스트가 여러개의 일을 하고 있을때 169.254.211.211 이라고 설정하고 다른 호스트에서도 같은 주소로 설정했을 때를 대비해서 DAD를 해야한다. 
    - DAD 필수
...
응답이 없으면 확정하고 ip 주소를 사용한다.

dns 서버 주소 뒤의 '%1'은 무얼 뜻하나요? 잘 모르겠다
