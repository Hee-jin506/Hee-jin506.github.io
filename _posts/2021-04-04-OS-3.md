...
- 단점
  ...
  - 한 모듈의 버그가 시스템 전체에 영향을 끼침
    - 여기서 의미하는 버그는 우리가 익히 아는 버그는 아니다. 그러한 버그는 이미 운영체제가 배포되는 시점에서 대부분 해결된 상태일 것이다. 여기에서 버그는 보안상의 취약점이 될 수  있는 지점을 말한다.
...
- 장점
  - 각 커널 서비스 서버가 따로 구현되어 있기 때문에 서로 간의 의존성이 낮음
    ...
    - 커널의 개발 및 유 보수가 상대적으로 용이
  - 커널 서비스 서버의 재시작/종료 가능
    - 불필요한 서비스의 서버는 종료
